...
def action_replay_after_end():
    global end_box
    if end_box != 0:
        end_box = 0
    action_start_game()

def action_menu_after_end():
    global end_box
    if end_box != 0:
        end_box = 0
    action_show_menu()

def action_do_fetch(t):
...
class TBtn:
    ...
    def oc(self, e):
        if self.kind == "home":
            ...
            action_sel_cat(self.idx)
        elif self.kind == "dif":
            action_sel_dif(self.idx)
        elif self.kind == "ans":
            action_chk_ans(self.idx)
        elif self.kind == "next":
            action_next()
        elif self.kind == "play":
            action_start_game()
            ...
            if end_box != 0:
                end_box._del()
            action_replay_after_end()
        elif self.kind == "menu":
            if end_box != 0:
                end_box._del()
            action_menu_after_end()
